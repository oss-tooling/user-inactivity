...
exports.report = (octokit) => async (org, repo, body) => {
  ...
  const [ orgMembers, usernames, outsideCollaborators ] = await Promise.all([
    // Get all members of the organization, with role member, in batches of 100
    octokit.paginate(octokit.orgs.listMembers, {
      org,
      role: 'member',
      per_page: 100
    }),
    // Get all open issues of the repository, in batches of 100
    octokit.paginate(octokit.issues.listForRepo, {
      ...
      // Extract the title of each issue
      (response) => response.data.map((issue) => issue.title.toLowerCase())
    ),
    // Get all outside collaborators of the organization, in batches of 100
  ...
  console.log(outsideCollaborators[0])
